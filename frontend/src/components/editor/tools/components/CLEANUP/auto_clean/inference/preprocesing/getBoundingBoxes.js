  ...
  extractTextBlock(canvas, background, pt_x, pt_y) {
    ...
    for (let y = 0; y < canvas.height; y++) {
      for (let x = 0; x < canvas.width; x++) {
        ...
        // Detecta píxeles de texto (negros)
        if (r === textValue && g === textValue && b === textValue) {
          if (x < left) left = x;
          if (x > right) right = x;
          if (y < top) top = y;
    ...
    if (w < 3 || h < 3) {
      console.warn("No se encontró bloque de texto significativo.");
      return null;
    }
  
    console.log("Bounding box:", { x: left, y: top, w, h });
