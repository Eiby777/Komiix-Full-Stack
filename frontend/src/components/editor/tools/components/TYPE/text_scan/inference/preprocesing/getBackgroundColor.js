...
export default function getBackgroundColor(croppedImages) {
  return croppedImages.map((canvasRectangles, canvasIndex) => {
    // If the inner array is empty, return an empty array for that index
    if (canvasRectangles.length === 0) {
      return [];
    }

    // Process each cropped image within this inner array
    return canvasRectangles.map(croppedImg => {
      const { canvas, id, coords, color, image } = croppedImg; // Removed canvasIndex as it's now implicit
      ...
      return {
        ...
        image,
        canvas,
        canvasIndex, // Keep canvasIndex for clarity in the output object
        background: {
          color: bgColor,
          rgb: bgRGB,
