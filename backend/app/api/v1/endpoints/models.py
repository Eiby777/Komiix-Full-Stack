...
METADATA_FILE = settings.MODEL_METADATA_FILE

FRAGMENTS_DIR = MODEL_DIR / "fragmented"
FULL_MODELS_DIR = MODEL_DIR / "full"

# Load MODEL_METADATA from JSON file
try:
    with open(METADATA_FILE, "r") as f:
        MODEL_METADATA = json.load(f)
    logger.info(f"MODEL_METADATA loaded from {METADATA_FILE}")
except FileNotFoundError:
    logger.error(f"Metadata file not found: {METADATA_FILE}")
    raise RuntimeError(f"Metadata file not found: {METADATA_FILE}")
except json.JSONDecodeError:
    logger.error(f"Error parsing {METADATA_FILE}")
    raise RuntimeError(f"Error parsing {METADATA_FILE}")
